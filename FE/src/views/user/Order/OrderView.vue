  ...
  <div class="container" v-if="mobile == false">
    <div class="row">
      <div class="col-12 text-center">
        <h4 class="pt-3">Đơn hàng của tôi</h4>
      </div>
    </div>
        ...
        <div class="card-block px-3">
          <h6 class="card-title">
            <router-link v-bind:to="'/order/' + order.id"
              >Mã đơn hàng: {{ order.id }}</router-link
            >
          </h6>
          <p class="mb-0">
          ...
          <p id="item-price" class="mb-0 font-weight-bold">
            Tổng cộng: {{ order.totalCost }}.000đ
          </p>
          <p id="item-total-price">Ngày đặt hàng: {{ order.orderdate }}</p>
        </div>
      </div>
      <div class="col-2"></div>
      <div class="col-12">
...
export default {
  ...
  mounted() {
    ...
    if (this.token) this.listOrders();
    else this.$router.replace("/mobilecheckouttrue");
  },
};
</script>
